...
fn cursor_pos_to_scroll_x_in_layout(cursor_pos int, shape &Shape, root &Layout, mut w Window) f32 {
	...
	cursor_left := shape_x_in_content - padding_left + text_layout_align_offset_x(shape, mut w) +
		rect.x
	cursor_right := cursor_left + f32_max(rect.width, 1)

	view_left := -current_scroll_x
	view_right := view_left + scroll_view_width

	mut target_scroll := current_scroll_x
	if cursor_left < view_left {
		target_scroll = -cursor_left
	} else if cursor_right > view_right {
		target_scroll = -(cursor_right - scroll_view_width)
	}
...
fn text_private_cursor_scroll_x(cursor_pos int, layout &Layout, current_x f32, mut w Window) ?f32 {
	...
	cursor_left := text_layout_align_offset_x(layout.shape, mut w) + rect.x
	cursor_right := cursor_left + f32_max(rect.width, 1)
	view_left := -current_x
	view_right := view_left + view_width
	mut target_x := current_x
	if cursor_left < view_left {
		target_x = -cursor_left
	} else if cursor_right > view_right {
		target_x = -(cursor_right - view_width)
	}
